...
const PropertyPanel: React.FC = () => {
  ...
  if (!selectedNode && !selectedEdge) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: 24 }}>
        <Empty description={<Text type="secondary" style={{ fontSize: 13 }}>点击画布中的节点或连线以编辑属性</Text>} />
      </div>
    );
  }
  ...
  if (selectedEdge && !selectedNode) {
    return (
      <div style={{ padding: '12px 12px 24px', overflow: 'auto', height: '100%' }}>
        <Title level={5} style={{ margin: '0 0 4px' }}>属性面板</Title>
        <Text type="secondary" style={{ fontSize: 12 }}>连线</Text>
        <Divider style={{ margin: '12px 0' }} />
        <Form form={form} layout="vertical" size="small" requiredMark={false}>
          <Form.Item label="连线文字" name="label">
            <Input placeholder="如 是 / 否" onChange={(e) => handleFieldChange('label', e.target.value)} />
          </Form.Item>
          <Form.Item label="备注" name="remark">
            <TextArea rows={2} placeholder="添加备注…" onChange={(e) => handleFieldChange('remark', e.target.value)} />
          </Form.Item>
        </Form>
        <Popconfirm title="确定删除该连线？" onConfirm={handleDelete} okText="删除" cancelText="取消">
          <Button danger icon={<DeleteOutlined />} block size="small">删除连线</Button>
        </Popconfirm>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '12px 12px 24px', overflow: 'auto', height: '100%' }}>
      <Title level={5} style={{ margin: '0 0 4px' }}>属性面板</Title>
      <Text type="secondary" style={{ fontSize: 12 }}>{nodeType} · {selectedNode!.shape}</Text>
      <Divider style={{ margin: '12px 0' }} />
      <Form form={form} layout="vertical" size="small" requiredMark={false}>
        <Form.Item label="文本内容" name="label">
          <Input placeholder="输入节点文字" onChange={(e) => handleFieldChange('label', e.target.value)} />
        </Form.Item>
        <Form.Item label="字号 (px)" name="fontSize">
          <InputNumber min={8} max={36} style={{ width: '100%' }} onChange={(val) => handleFieldChange('fontSize', val ?? 12)} />
        </Form.Item>
        <Form.Item label="填充色" name="color">
          <ColorPicker format="hex" showText onChange={(_, hex) => handleFieldChange('color', hex)} />
        </Form.Item>
        <Form.Item label="边框色" name="borderColor">
          <ColorPicker format="hex" showText onChange={(_, hex) => handleFieldChange('borderColor', hex)} />
        </Form.Item>
        <Form.Item label="字体颜色" name="fontColor">
          <ColorPicker format="hex" showText onChange={(_, hex) => handleFieldChange('fontColor', hex)} />
        </Form.Item>
        <Form.Item label="备注" name="remark">
          <TextArea rows={3} placeholder="添加备注信息…" onChange={(e) => handleFieldChange('remark', e.target.value)} />
        </Form.Item>
      </Form>
      <Popconfirm title="确定删除该节点？" onConfirm={handleDelete} okText="删除" cancelText="取消">
        <Button danger icon={<DeleteOutlined />} block size="small">删除节点</Button>
      </Popconfirm>
    </div>
  );
};
