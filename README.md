...
├── src/
│   ├── main/                          # Electron 主进程
│   │   └── index.ts                   # 窗口创建、IPC 监听、自动保存、最近文件
...
│           ├── components/
│           │   ├── graph-editor/      # X6 画布组件 + 右键菜单 + 搜索
│           │   ├── sidebar/           # 左侧节点面板 + 连线样式 + 搜索过滤
│           │   ├── toolbar/           # 顶部工具栏 + 撤销重做 + 复制粘贴 + 缩放 + 最近文件
│           │   ├── property-panel/    # 右侧属性面板
│           │   ├── sheet-tabs/        # 多 Sheet 标签页管理
│           │   └── alignment-toolbar/ # 节点对齐/分布工具栏
│           ├── stores/
│           │   ├── useGraphStore.ts   # 图数据状态管理 + Sheet 管理 + 对齐操作
...
测试覆盖核心 store 逻辑（useHistoryStore、useGraphStore）。
